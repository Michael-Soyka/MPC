...
void VPC_Config_Keyword(configKeyword_e keyword, const char *pkeywordToken) {
  ...
  if (!g_pVPC->GetProjectGenerator()->StartPropertySection(keyword,
                                                           &bShouldSkip)) {
    g_pVPC->VPCSyntaxError("Unsupported Keyword: %s for target platform",
                           pkeywordToken);
  }
  ...
  if (bShouldSkip) {
    ...
    while (1) {
      ...
      // Copy off the token name so HandleProperty() doesn't have to (or else
      // the parser will overwrite it on the next token).
...
void VPC_Keyword_Configuration() {
  ...
  if (!configs.Count()) {
    ...
    if (!configs.Count()) {
      g_pVPC->VPCError(
          "Trying to parse a configuration block and no configs have been "
          "defined yet.\n[%s line:%d]",
...
void VPC_Keyword_FolderConfiguration(folderConfig_t *pFolderConfig) {
  ...
  if (pFolderConfig->vecConfigurationNames.Count() == 0) {
    g_pVPC->GetProjectGenerator()->GetAllConfigurationNames(
        pFolderConfig->vecConfigurationNames);
  }
  ...
  // save parser state so we remember where the block is.  We'll refer back to
  // it when handlign individual files.
...
void VPC_ApplyFolderConfigurationToFile(const folderConfig_t &folderConfig) {
  ...
  for (intp i = 0; i < folderConfig.vecConfigurationNames.Count(); i++) {
    ...
    // file.
    g_pVPC->GetProjectGenerator()->StartConfigurationBlock(
        folderConfig.vecConfigurationNames[i].String(), true);
