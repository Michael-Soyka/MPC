...
void CProjectGenerator_CodeLite::GenerateCodeLiteProject(
  ...
  g_pVPC->VPCStatus(true, "Saving CodeLite project for: '%s' File: '%s'",
                    pCollector->GetProjectName().String(), szProjectFile);
        ...
        pCollector->GetProjectName().String());
  {
    ...
    Write("<Settings Type=\"Dynamic Library\">\n");
    {
      ...
      Write(
          ...
          "DebuggerType=\"GNU gdb debugger\" Type=\"Dynamic Library\" "
          ...
          "BuildResWithGlobalSettings=\"append\">\n");
      {
        ...
        Write("<CustomBuild Enabled=\"yes\">\n");
        {
          ++m_nIndent;
          Write(
              "<RebuildCommand>make CFG=debug -f %s clean "
              "all</RebuildCommand>\n",
              pMakefileFilename);
          Write("<CleanCommand>make CFG=debug -f %s clean</CleanCommand>\n",
                pMakefileFilename);
          Write(
              "<BuildCommand>make CFG=debug -f %s -j `getconf "
              "_NPROCESSORS_ONLN`</BuildCommand>\n",
              pMakefileFilename);
          ...
          "BuildResWithGlobalSettings=\"append\">\n");
      {
        ...
        Write("<CustomBuild Enabled=\"yes\">\n");
        {
          ...
          Write("<CleanCommand>make -f %s clean</CleanCommand>\n",
                pMakefileFilename);
          Write(
              "<BuildCommand>make -f %s -j `getconf "
              "_NPROCESSORS_ONLN`</BuildCommand>\n",
              pMakefileFilename);
          Write("<WorkingDirectory>$(ProjectPath)</WorkingDirectory>\n");
          --m_nIndent;
    ...
    {
      ++m_nIndent;
      WriteFilesFolder(k_pchSource,
                       "*.c;*.C;*.cc;*.cpp;*.cp;*.cxx;*.c++;*.prg;*.pas;*.dpr;*"
                       ".asm;*.s;*.bas;*.java;*.cs;*.sc;*.e;*.cob;*.html;*.rc;*"
                       ".tcl;*.py;*.pl;*.m;*.mm");
...
void CProjectGenerator_CodeLite::WriteFilesFolder(const char *pFolderName,
  ...
  {
    ++m_nIndent;
    for (int i = m_pCollector->m_Files.First();
         i != m_pCollector->m_Files.InvalidIndex();
         i = m_pCollector->m_Files.Next(i)) {
