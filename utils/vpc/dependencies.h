...
#ifndef VPC_DEPENDENCIES_H_
#define VPC_DEPENDENCIES_H_

enum EDependencyType {
  k_eDependencyType_SourceFile,  // .cpp, .cxx, .h, .hxx
  k_eDependencyType_Project,     // this is a project file WITHOUT the
                                 // target-specific extension (.mak, .vpj,
                                 // .vcproj).
  k_eDependencyType_Library,     // this is a library file
  k_eDependencyType_Unknown  // Unrecognized file extension (probably .ico or
                             // .rc2 or somesuch).
};
...
#define BUILDPROJDEPS_FULL_DEPENDENCY_SET \
...
class CDependency {
 ...
 public:
  ...
  bool DependsOn(CDependency *pTest,
                 int flags = k_EDependsOnFlagCheckNormalDependencies |
                             k_EDependsOnFlagRecurse);
  const char *GetName() const;

  // Returns true if the absolute filename of this thing
  // (CDependency::m_Filename) matches the absolute path specified.
  bool CompareAbsoluteFilename(const char *pAbsPath) const;

 private:
  bool FindDependency_Internal(CUtlVector<CUtlBuffer> &callTreeOutputStack,
                               CDependency *pTest, int flags, int depth);
  void Mark();
  bool HasBeenMarked();

 public:
  CUtlString m_Filename;  // Full paths (slashes are platform dependent).
                          // This is the VPC filename for a project (use
                          // CDependency_Project::m_ProjectFilename for the
                          // VCPROJ/VPJ filename).
  ...
  // Files added by $AdditionalProjectDependencies. This is in a separate list
  // because we don't always want DependsOn() to check this.
  CUtlVector<CDependency *> m_AdditionalDependencies;

 private:
  CProjectDependencyGraph *m_pDependencyGraph;
  unsigned int m_iDependencyMark;
  bool m_bCheckedIncludes;  // Set to true when we have checked all the includes
                            // for this.
...
// This represents a project (.vcproj) file, NOT a project like a
// projectIndex_t. There can be separate .vcproj files (and thus separate
// CDependency_Project) for each game and platform of a projectIndex_t. If
// m_Type == k_eDependencyType_Project, then you can cast to this.
class CDependency_Project : public CDependency {
 public:
  ...
  // These functions read/write g_pVPC->GetOutputFilename() and such (all the
  // m_xxStoredXXXX vars below).
 ...
 public:
  ...
  // Straight out of the $AdditionalProjectDependencies key (split on
  // semicolons).
  ...
  // This is used by /p4sln. It uses this to call into VPC_ParseProjectScript.
  // These are the values of g_pVPC->GetOutputFilename(), szScriptName, and the
  // defines at the time of building this project.
...
class CProjectDependencyGraph : public IProjectIterator {
 ...
 public:
  ...
  void BuildProjectDependencies(
      ...
      CUtlVector<CDependency_Project *> *pPhase1Projects = NULL);
  ...
  // Look for all projects (that we've scanned during BuildProjectDependencies)
  // that depend on the specified project. If bDownwards is true,  then it adds
  // iProject and all projects that _it depends on_. If bDownwards is false,
  // then it adds iProject and all projects that _depend on it_.
  void GetProjectDependencyTree(projectIndex_t iProject,
                                CUtlVector<projectIndex_t> &dependentProjects,
                                bool bDownwards);

  // This solves the central mismatch between the way VPC references projects
  // and the way the CDependency stuff does.
  //
  // - VPC uses projectIndex_t, but a single projectIndex_t can turn into
  // multiple games (server_tf, server_episodic, etc) in
  // VPC_IterateTargetProjects.
  // - The dependency code has a separate CDependency_Project for each game.
  //
  // This takes a bunch of project indices (usually m_targetProjects, which
  // comes from the command line's "+this -that *theother" syntax), which are
  // game-agnostic, and based on what games were specified on the command line,
  // it builds the list of CDependency_Project*s.
  void TranslateProjectIndicesToDependencyProjects(
 ...
 public:
  ...
  CUtlDict<CDependency *, int>
      m_AllFiles;  // All files go in here. They should never be duplicated.
                   // These are indexed by the full filename (except .lib files,
                   // which have that stripped off).
  bool m_bFullDependencySet;  // See bFullDepedencySet passed into
 ...
 private:
  ...
  unsigned int m_iDependencyMark;
  bool m_bHasGeneratedDependencies;  // Set to true after finishing
                                     // BuildProjectDependencies.
};
