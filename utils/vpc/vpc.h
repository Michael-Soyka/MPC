...
struct conditional_t {
  ...
  // only used during multiple game iterations for game conditionals as each
  // 'defined' game becomes active
...
struct macro_t {
  ...
  // If set to true, then VPC will add this as a -Dname=value parameter to the
  // compiler's command line.
  bool m_bSetupDefineInProjectFile;

  // VPC created this macro itself rather than the macro being created from a
  // script file.
...
class CVPC {
 public:
  ...
  void SetPhase1Projects(CUtlVector<CDependency_Project *> *pPhase1Projects) {
    m_pPhase1Projects = pPhase1Projects;
  }

  IBaseProjectGenerator *GetProjectGenerator() { return m_pProjectGenerator; }
  void SetProjectGenerator(IBaseProjectGenerator *pGenerator) {
    m_pProjectGenerator = pGenerator;
  }

  IBaseSolutionGenerator *GetSolutionGenerator() {
    return m_pSolutionGenerator;
  }

  // Conditionals
  conditional_t *FindOrCreateConditional(const char *pName, bool bCreate,
                                         conditionalType_e type);
  void ResolveMacrosInConditional(char const *pString, char *pOutBuff,
                                  int outBuffSize);
  ...
  // Macros
  macro_t *FindOrCreateMacro(const char *pName, bool bCreate,
                             const char *pValue);
  ...
  void SetMacro(const char *pName, const char *pValue,
                bool bSetupDefineInProjectFile);

  // Iterates all the projects in the specified list, checks their conditionals,
  // and calls pIterator->VisitProject for each one that passes the conditional
  // tests.
  //
  ...
  bool ParseProjectScript(const char *pScriptName, int depth, bool bQuiet,
 ...
 private:
  ...
  void ResolveMacrosInStringInternal(char const *pString, char *pOutBuff,
                                     int outBuffSize,
                                     bool bStringIsConditional);
  ...
  void CreateOutputFilename(project_t *pProject, const char *pchPlatform,
  ...
  bool BuildTargetProject(IProjectIterator *pIterator,
                          projectIndex_t projectIndex, script_t *pProjectScript,
...
extern const char
    *g_pOption_AdditionalIncludeDirectories;  // "$AdditionalIncludeDirectories"
extern const char *
    g_pOption_AdditionalProjectDependencies;  // "$AdditionalProjectDependencies"
extern const char *g_pOption_AdditionalOutputFiles;  // "$AdditionalOutputFiles"
extern const char
    *g_pOption_PreprocessorDefinitions;  // "$PreprocessorDefinitions"
extern const char *g_IncludeSeparators[2];

extern void VPC_ParseGroupScript(const char *pScriptName);

extern groupTagIndex_t VPC_Group_FindOrCreateGroupTag(const char *pName,
                                                      bool bCreate);
...
extern void VPC_FakeKeyword_SchemaFolder(
    class CBaseProjectDataCollector *pDataCollector);
