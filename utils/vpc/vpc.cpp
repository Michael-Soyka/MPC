...
const char *g_pOption_AdditionalIncludeDirectories =
    "$AdditionalIncludeDirectories";
const char *g_pOption_AdditionalProjectDependencies =
...
bool CVPC::Init(int argc, const char **argv) {
  ...
  Log_Msg( LOG_VPC, Color( 0, 255, 255, 255 ), "MPC - Moon Project Creator\n" );
  Log_Msg( LOG_VPC, "for Visual Studio, Xcode and Make\n" );
  Log_Msg( LOG_VPC, "Build time: %s %s; Build commit: %s\n", __DATE__, __TIME__, GIT_SHA );
  Log_Msg( LOG_VPC, "(C) Copyright 1996-2026 Valve Corporation, 2020-2026 Moon-6 Team. All rights reserved.\n" );
  Log_Msg( LOG_VPC, "\n" );
...
void CVPC::SpewUsage(void) {
  ...
  bool bNoHeader = m_bSpewPlatforms || m_bSpewGames || m_bSpewProjects ||
                   m_bSpewGroups || m_bSpewProperties ||
  ...
  if (!bNoHeader) {
    ...
    if (!m_bHelp) {
      ...
      Log_Msg(LOG_VPC,
              "[/testmode]:   Override output .vcproj file to be named "
              "'test.vcproj'\n");
#ifdef VPC_SCC_INTEGRATION
      Log_Msg(LOG_VPC,
      ...
      Log_Msg(LOG_VPC,
              ...
              "Studio 2015\n");
      Log_Msg(LOG_VPC,
              "[/2013]:       Generate projects and solutions for Visual "
              "Studio 2013\n");
      Log_Msg(LOG_VPC,
              "[/2012]:       Generate projects and solutions for Visual "
              "Studio 2012\n");
      Log_Msg(LOG_VPC,
      ...
      Log_Msg(LOG_VPC,
              "[/32bittools]: Specify 32-bit toolchain in VC++ even when "
              "compiling 64 bit target\n");
      ...
      Log_Msg(LOG_VPC,
              "[/p4sln]:      <.sln filename> <changelists...> - make a "
              "solution file based on\n");
      Log_Msg(LOG_VPC,
              "               the changelist. Changelists can be specific "
              "numbers, 0 or \"default\"\n");
      Log_Msg(LOG_VPC,
              "               for the default changelist, or \"all\" for all "
              "active changelists.\n");
      Log_Msg(
          LOG_VPC,
          "[/nop4add]:    Don't automatically add project files to Perforce\n");
      Log_Msg(LOG_VPC,
              "[/slnitems]:   <filename> - adds all files listed in <filename> "
              "to generated\n");
      Log_Msg(LOG_VPC, "               solutions\n");
      Log_Msg(LOG_VPC,
              "[/showdeps]:   Show an example dependency chain for each "
...
void CVPC::HandleSingleCommandLineArg(const char *pArg) {
  if ((pArg[0] == '-') || (pArg[0] == '/')) {
    ...
    // check options
    if (!V_stricmp(pArgName, "h") || !V_stricmp(pArgName, "?") ||
        !V_stricmp(pArgName, "help")) {
      m_bHelp = true;
      m_bUsageOnly = true;
    } else if (!V_stricmp(pArgName, "v") || !V_stricmp(pArgName, "verbose")) {
      m_bVerbose = true;
    } else if (!V_stricmp(pArgName, "testmode") ||
               !V_stricmp(pArgName, "test")) {
      m_bTestMode = true;
    } else if (!V_stricmp(pArgName, "f") || !V_stricmp(pArgName, "force")) {
      m_bForceGenerate = true;
    } else if (!V_stricmp(pArgName, "no_posix_pch")) {
      // Not implemented for win32 generators since those just pass through raw
      // options :-/
      m_bNoPosixPCH = true;
      ...
      m_bCheckFiles = false;
    } else if (!V_stricmp(pArgName, "showcaseissues") ||
               !V_stricmp(pArgName, "showcase")) {
      m_bShowCaseIssues = true;
    } else if (!V_stricmp(pArgName, "dp")) {
      m_bDecorateProject = true;
    } else if (char const *szActualDecorateName =
                   StringAfterPrefix(pArgName, "decorate:")) {
      m_bDecorateProject = true;
      ...
      m_bUseValveBinDir = true;
    } else if (!V_stricmp(pArgName, "platforms") ||
               !V_stricmp(pArgName, "plats")) {
    ...
    } else if (char const *szActualDefineName =
                   StringAfterPrefix(pArgName, "define:")) {
      // allow setting custom defines straight from command line
      conditional_t *pConditional =
          FindOrCreateConditional(szActualDefineName, true, CONDITIONAL_CUSTOM);
      if (pConditional) {
        pConditional->m_bDefined = true;

        m_ExtraOptionsCRCString +=
            "/define:";  // force this into additional CRC string
        m_ExtraOptionsCRCString +=
            pConditional->name.Get();  // force this into additional CRC string
      }
    } else if (!V_stricmp(pArgName, "32bittools")) {
      m_b32BitTools =
          true;  // use 32-bit toolchain even when building 64-bit targets
      m_ExtraOptionsCRCString +=
          pArgName;  // Make sure these options affect the CRC.
...
void CVPC::ParseBuildOptions(int argc, const char *argv[]) {
  ...
  // parse options
  // prefer +??? or -??? prefix syntax for groups and /??? for options because
  // less confusing for new vpc users for ease we will support -??? prefix
  // syntax for matched options as well
  for (int i = 1; i < argc; i++) {
    ...
    if (!V_stricmp(pArg, "/mksln")) {
      ...
      // If the next parameter is a standard + or - or / or * parameter, then we
      // take that to be the name of the solution file. So vpc /mksln +engine
      // would generate engine.sln.
      ...
      if (i >= argc || argv[i][0] == '+' || argv[i][0] == '-' ||
        ...
        VPCError(
            "%s <solution filename> <changelist number> [ [restrict_to_group] "
            "].",
            pArg);
      }
      ...
      if (m_iP4Changelists.Count() == 0) {
        VPCError(
            "%s <solution filename> <changelist number> [additional changelist "
            "numbers] [ [restrict_to_group] ].",
            pArg);
      }
    ...
    else if ( !V_stricmp( pArg, "/scriptsdir" ) )
    {
      ...
      if (!m_OutputMirrorString.IsEmpty() &&
          !V_IsAbsolutePath(m_OutputMirrorString.Get())) {
  ...
  // If they did /p4sln but didn't specify any build commands, then have it
  // check everything.
...
void CVPC::GenerateOptionsCRCString() {
  ...
  conditional_t *pConditional =
      FindOrCreateConditional("PROFILE", false, CONDITIONAL_NULL);
...
void CVPC::CreateOutputFilename(project_t *pProject, const char *pchPlatform,
                                const char *pchPhase, const char *pGameName,
                                const char *pchExtension) {
  const char *pProjectFileNamePrefix =
      m_bTestMode ? "test" : pProject->name.String();
...
bool CVPC::BuildTargetProject(IProjectIterator *pIterator,
                              projectIndex_t projectIndex,
                              script_t *pProjectScript, const char *pGameName) {
  ...
  // win32 projects are the most prevalent, so by popular demand they have no
  // decoration all other platforms use their platform name as a suffix
  ...
  CreateOutputFilename(
      pProject, pPlatformName, pPhaseName, pGameName,
  ...
  // this allows vpc to be invoked anywhere, but the groups resolve their
  // projects correctly
  char szScriptPath[MAX_PATH];
  V_ComposeFileName(g_pVPC->GetStartDirectory(), pProjectScript->name.String(),
                    szScriptPath, sizeof(szScriptPath));
...
//-----------------------------------------------------------------------------
void CVPC::IterateTargetProjects(CUtlVector<projectIndex_t> &projectList,
  ...
  for (intp nProject = 0; nProject < projectList.Count(); nProject++) {
    ...
    for (intp nScript = 0; nScript < pProject->scripts.Count(); nScript++) {
      ...
      bool bHasGameCondition = g_pVPC->ConditionHasDefinedType(
          pProjectScript->m_condition.String(), CONDITIONAL_GAME);
      ...
      if (!bHasGameCondition) {
        ...
        for (intp nTargetGame = 0; nTargetGame < m_Conditionals.Count();
          ...
          BuildTargetProject(pIterator, projectList[nProject], pProjectScript,
                             m_Conditionals[nTargetGame].name.String());
        }
      }
    }
  }
...
bool CVPC::BuildTargetProjects() {
   ...
   public:
    virtual bool VisitProject(projectIndex_t, const char *pScriptPath) {
      ...
      if (!g_pVPC->IsForceGenerate() &&
          g_pVPC->IsProjectCurrent(g_pVPC->GetOutputFilename(), true) &&
          !g_pVPC->IsForceIterate()) {
  ...
  if (!m_TargetProjects.Count()) {
    VPCError(
        "No recognized project(s) to build.  Use /h or /projects or /groups to "
        "spew more info.");
  }

  /* At this point we might've built the project dependency graph, which
   * increments the missing file count if *any* project in your VPC directory
   * has a missing file, so we reset it here to get the accurate count.
   */
  ...
  if (!m_bGeneratedProject && !m_bAnyProjectQualified) {
    ...
    Log_Warning(LOG_VPC, Color(255, 255, 0, 255), "\n");
    Log_Warning(LOG_VPC, Color(255, 255, 0, 255),
                "Possibly missing game, platform, or other conditional "
                "expected by script.\n");
    Log_Warning(LOG_VPC, Color(255, 255, 0, 255),
...
void CVPC::SetMacrosAndConditionals() {
  ...
  for (intp i = 0; i < m_Conditionals.Count(); i++) {
    if (&m_Conditionals[i] != pPlatformConditional &&
        ...
        m_Conditionals[i].m_bDefined) {
      // no no no, the user is not allowed to build multiple platforms
      // simultaneously this prior feature really confused/crapped up the code,
      // so absolutely not supporting that
      VPCWarning("Detected multiple target platforms...Disabling '%s'",
                 m_Conditionals[i].name.String());
      m_Conditionals[i].m_bDefined = false;
    }
  }

  if (!pPlatformConditional) {
    // no user specified platform defined, defaults to primary vpc.exe built
    // platform
#if defined(WIN32)
...
#error "Unsupported platform."
  ...
  // Cache the platform name so that we can use it without dereferencing
  // pPlatformConditional
  ...
  // It really should be called something else that won't conflict with the rest
  // of the world.
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVPC::HasP4SLNCommand() { return HasCommandLineParameter("/p4sln"); }
...
bool CVPC::HandleP4SLN(
  ...
  if (m_iP4Changelists.Count() == 0) return false;
  ...
  if (V_IsAbsolutePath(m_P4SolutionFilename.Get())) {
    V_strncpy(szFullSolutionPath, m_P4SolutionFilename.Get(),
              sizeof(szFullSolutionPath));
  } else {
    V_ComposeFileName(g_pVPC->GetStartDirectory(), m_P4SolutionFilename.Get(),
                      szFullSolutionPath, sizeof(szFullSolutionPath));
  }

  CProjectDependencyGraph dependencyGraph;
  GenerateSolutionForPerforceChangelist(dependencyGraph, m_iP4Changelists,
                                        pSolutionGenerator, szFullSolutionPath);
  return true;
#else
  return false;
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CVPC::GetProjectDependencies(
...
void CVPC::SetupGenerators() {
  ...
  // specified a linux config, or if they're building the (non-SRCDS) dedicated
  // server, then use the makefile generator
  conditional_t *pConditional =
...
//-----------------------------------------------------------------------------
// Since Steam's VPC builds tier0 and vstdlib directly in, Steam uses vpc.exe as
...
const char *CVPC::BuildTempGroupScript(const char *pVPCScriptName) {
  ...
  V_ComposeFileName(szCurrentDirectory, pVPCScriptName, vpcScriptFilename,
                    sizeof(vpcScriptFilename));

  // the actual vpc must be relative to the source path
  const char *pVPCFilename =
      StringAfterPrefix(vpcScriptFilename, m_SourcePath.Get());
  if (!pVPCFilename) {
