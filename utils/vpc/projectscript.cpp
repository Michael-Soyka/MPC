...
void VPC_ParseFileSection(bool *pbHadConfigSection = NULL) {
  if (pbHadConfigSection) *pbHadConfigSection = false;

  while (1) {
    const char *pToken = g_pVPC->GetScript().GetToken(true);
    if (!pToken || !pToken[0]) break;
    ...
    if (!V_stricmp(pToken, "$configuration")) {
      ...
      if (pbHadConfigSection) *pbHadConfigSection = true;
    }
  }
}
...
void VPC_TrackSchemaFile(const char *pName, bool bRemove,
                         const char *pFileFlag) {
#ifdef STEAM
  return;
#else
...
static char *ResolveCandidate(const char *pszFile, const char *pszPlatform) {
  ...
  Sys_ReplaceString(szPath, "$os", pszPlatform, szPathExpanded,
  ...
  if (Sys_Exists(szPathExpanded)) {
    ...
    Sys_ReplaceString(pszFile, "$os", pszPlatform, pszResolvedFilename,
                      MAX_PATH);
...
// No particular order, but must not change without changing the
...
//		Returns expanded filename to a file that exists on disk, or NULL
// otherwise 		vecBonusFiles will contain a list of valid OS files that
// are on disk but did not 		match the current target platform.
//-----------------------------------------------------------------------------
static char *ResolveFilename(const char *pszFile,
  ...
  // compile-time init'ing, but it is single-instance initialized, so make a
  // const ref to it that everyone except the init codes uses to make sure it
  // doesn't get mucked with.
  ...
  if (nPlatformColumn < 0) {
    g_pVPC->VPCWarning(
        "Internal Error: Target Platform: '%s' unrecognized while expanding "
        "$os!",
        szPlatform);
    return NULL;
  }
  ...
  do {
    ...
    if (pszResolved) break;
  } while (arrPlatformChains[nCurrentPlatformRow][nPlatformColumn] !=
           k_AnyPlatform);
  ...
  for (int i = 0; g_szArrPlatforms[i] != NULL; i++) {
    ...
    if (i != nPlatformToCheck) {
      ...
      if (!file.IsEmpty()) vecBonusFiles.AddToTail(file);
    }
  }
  return pszResolved;
}
    ...
    if (pToken[0] == '[') {
      if (files.Count() == 0) {
        g_pVPC->VPCSyntaxError(
            "Conditional specified on a $FilePattern without any pattern "
            "preceding it.");
      }
...
void VPC_Keyword_Folder(bool bUnity,
                        const folderConfig_t *pInheritedFolderConfig = NULL);
...
void VPC_Keyword_AddFile(const char *pFileFlag = NULL,
                         const folderConfig_t *pFolderConfig = NULL) {
  ...
  while (1) {
    ...
    if (pToken[0] == '[') {
      if (files.Count() == 0) {
        g_pVPC->VPCSyntaxError(
            "Conditional specified on a $File without any file preceding it.");
      }
  ...
  if (g_pVPC->IsCheckFiles() && !bDynamicFile) {
    for (intp i = 0; i < files.Count(); i++) {
      ...
      if (!Sys_Exists(pFilename) && !V_stristr(pFilename, "$os")) {
#if defined(POSIX)
        // We have a _lot_ of vpc files that contain header files with the
        // incorrect casing. So try to lowercase the filename here and if it
        // exists, replace it and carry on.
        //
        ...
        // Hack around all the libraries which are listed as $File instead of
        // $Lib
        const char *extension = strrchr(pFilename, '.');
        if (!extension || stricmp(extension, ".lib")) {
          g_pVPC->VPCWarning("File '%s' does not exist. Not adding to project.",
                             pFilename);
          g_pVPC->IncrementFileMissing();
          files.Remove(i);
        } else {
          // don't complain about libs that contain $(Configuration) in their
          // path
          if (!V_stristr(pFilename, "$(Configuration)"))
            g_pVPC->VPCWarning(
                "Library '%s' does not exist. Adding to project anyway.",
                pFilename);
        }
      }
    }
  ...
  if (g_pVPC->IsShowCaseIssues() && !bDynamicFile) {
    for (intp i = 0; i < files.Count(); i++) {
      ...
      if (!Sys_IsFilenameCaseConsistent(pFilename, actualFilename,
                                        sizeof(actualFilename))) {
        g_pVPC->VPCWarning(
            "Case Consistency Issue! File '%s' specified in '%s' is "
            "inconsistent with OS version '%s'.",
            pFilename, g_pVPC->GetProjectName(), actualFilename);
  ...
  for (intp k = 0; k < files.Count(); k++) {
    ...
    if (!g_pVPC->m_bInMkSlnPass && V_stristr(pFilename, "$os")) {
      ...
      if (filename.IsEmpty()) {
        g_pVPC->VPCWarning(
            "$File %s did not resolve to an existing file, skipping!",
            pFilename);
        continue;
      }

      for (const auto &excludedFile : vecExcludedFiles) {
        const char *pExcludedFilename = excludedFile.String();
    ...
    if (!g_pVPC->m_sUnityCurrent.IsEmpty() && !V_stricmp(pExtension, "cpp") &&
      ...
      // if this file doesn't have a file-specific config, but the folder we're
      // in has a config
    ...
    if (bAdded) g_pVPC->GetProjectGenerator()->EndFile();
  }
}

// parse a list of filenames from the current token, handling conditionals,
// macro expansion, \\, fixslashes, etc
...
// add or remove .lib or dll import files, automatically adding prefixes and
// suffices to the name
static void VPC_HandleLibraryExpansion(char const *pDefaultPath,
                                       char const *pFileNamePrefix,
                                       char const *pSuffix, bool bRemove) {
  if (!pFileNamePrefix) {
    pFileNamePrefix = "";
  }
  char szResolvedFilePrefix[MAX_PATH];
  g_pVPC->ResolveMacrosInString(pFileNamePrefix, szResolvedFilePrefix,
                                sizeof(szResolvedFilePrefix));
  ...
  for (intp i = 0; i < impFiles.Count(); i++) {
    ...
    // already, or if null was passed.
    char impFile[MAX_PATH];
    V_strncpy(impFile, impFiles[i], sizeof(impFile));
    V_RemoveDotSlashes(impFile);

    char *pLastSlash =
        (char *)MAX(strrchr(impFile, '\\'), strrchr(impFile, '/'));
    if (pLastSlash) {
      *pLastSlash = 0;
      const char *pFilenamePart = pLastSlash + 1;
      // don't prepend the prefix if the filename already starts with it
      bool bAddPrefix = true;
      if (V_strlen(szResolvedFilePrefix) &&
          !V_strnicmp(pFilenamePart, szResolvedFilePrefix,
                      V_strlen(szResolvedFilePrefix)))
        bAddPrefix = false;
      sprintf(szFilename, "%s/%s%s%s", impFile,
              bAddPrefix ? pFileNamePrefix : "", pFilenamePart, pSuffix);
    } else {
      bool bAddPrefix = true;
      if (V_strlen(szResolvedFilePrefix) &&
          !V_strnicmp(impFiles[i], szResolvedFilePrefix,
                      V_strlen(szResolvedFilePrefix)))
        bAddPrefix = false;
      sprintf(szFilename, "%s%s%s%s", pPathPrefixToUse,
              bAddPrefix ? pFileNamePrefix : "", impFiles[i], pSuffix);
    }
    ...
    if (bRemove) {
      ...
      if (!bSucc) {
        g_pVPC->VPCError(
            "Broken $implib command. Failed to remove file %s from project.",
            szFilename1);
      }
    } else {
...
static void VPC_Keyword_ImportLibrary(uint32 Flags) {
  ...
  VPC_HandleLibraryExpansion("$LIBPUBLIC\\", "$_IMPLIB_PREFIX", pSuffix,
                             bRemove);
}
...
static void VPC_Keyword_LinkerLibrary(uint32 Flags) {
  ...
  VPC_HandleLibraryExpansion("$LIBPUBLIC\\", "$_STATICLIB_PREFIX", pSuffix,
                             bRemove);
}
...
void VPC_Keyword_Folder(
    bool bUnity, const folderConfig_t *pInheritedFolderConfig /* = NULL */) {
  const char *pToken;
  char folderName[MAX_PATH];
  folderConfig_t folderConfig;

  // by default, our active config is any config that we inherited from the
  // parent.
  const folderConfig_t *pActiveFolderConfig = pInheritedFolderConfig;

  if (!g_pVPC->GetScript().ParsePropertyValue(NULL, folderName,
                                              sizeof(folderName))) {
    g_pVPC->GetScript().SkipBracedSection();
    return;
  }
  ...
  if (bUnity) {
    ...
    int cAttempt = 1;
    while (g_pVPC->m_UnityFilesSeen.Find(g_pVPC->m_sUnityCurrent) !=
    ...
    if (bEmitUnityFiles) {
      g_pVPC->VPCStatus(false, "Unity: emitting '%s' in project: '%s'",
                        unityName, g_pVPC->GetProjectName());

      // always add the stdafx.h at the top (if we're using a precompiled
      // header, what if we're not?)
      FILE *fp = fopen(g_pVPC->m_sUnityCurrent.String(), "wt");
      if (!fp) {
        g_pVPC->VPCError("Cannot open %s for writing",
                         g_pVPC->m_sUnityCurrent.String());
      } else {
  ...
  while (1) {
    ...
    if (!V_stricmp(pToken, "}")) {
      ...
      break;
               ...
               !V_stricmp(pToken, "$schemafile") ||
               !V_stricmp(pToken, "$entschemafile")) {
      // add file
    ...
    //		}
      ...
      // if we found a new config section, make it active and apply it to
      // subsequent files
...
void VPC_Keyword_Shaders(int) {
  ...
  if (!fxcList.Count() && !vshList.Count() && !pshList.Count() &&
      !vfxList.Count() && !otherList.Count()) {
...
void VPC_Keyword_Macro(MacroType_t eMacroType) {
  ...
  if (Sys_EvaluateEnvironmentExpression(value, "", environmentValue,
                                        sizeof(environmentValue))) {
    V_strncpy(value, environmentValue, sizeof(value));
  }

  g_pVPC->FindOrCreateMacro(macro, true,
                            (eMacroType == VPC_MACRO_VALUE) ? value : "");
}
...
//-----------------------------------------------------------------------------
enum MacroRequiredType_t {
  VPC_MACRO_REQUIRED_NOT_EMPTY,
  VPC_MACRO_REQUIRED_ALLOW_EMPTY
};
void VPC_Keyword_MacroRequired(MacroRequiredType_t eMacroRequiredType) {
  ...
  if (pToken && pToken[0]) {
    if (pToken[0] == '[') {
      ...
      if (!g_pVPC->GetScript().ParsePropertyValue(NULL, macroDefaultValue,
                                                  sizeof(macroDefaultValue))) {
        return;
      }
    }
  }
  ...
  if (!pMacro || (eMacroRequiredType == VPC_MACRO_REQUIRED_NOT_EMPTY &&
                  !strlen(pMacro->value.String()))) {
    if (macroDefaultValue[0] ||
        (eMacroRequiredType == VPC_MACRO_REQUIRED_ALLOW_EMPTY)) {
      g_pVPC->FindOrCreateMacro(macroName, true, macroDefaultValue);
    } else {
      // In case we're in mksln showing a pacifier of dots. Make sure to show
      // the error on a new line.
      g_pVPC->VPCSyntaxError("\n\nRequired Macro '%s', not defined or empty",
                             macroName);
    }
  }
}
...
//	}
//
//	Specialized instruction to populate the load address macro based on a
// project 	name.
//-----------------------------------------------------------------------------
void VPC_Keyword_LoadAddressMacro(void) {
  ...
  if (!g_pVPC->GetScript().ParsePropertyValue(NULL, szMacroName,
                                              sizeof(szMacroName))) {
    g_pVPC->GetScript().SkipBracedSection();
    return;
  }
  ...
  while (1) {
    ...
    if (!V_stricmp(pToken, "}")) {
      ...
      if (!g_pVPC->GetScript().ParsePropertyValue(NULL, szBaseAddress,
                                                  sizeof(szBaseAddress))) {
        continue;
      }
...
//	}
//
//	When evaluating $LoadAddressMacro/$LoadAddressMacroAuto, substitute all
// listed <ProjectName> entries with <Alias>
//-----------------------------------------------------------------------------
...
//
//	bPad - Differentiate between $LoadAddressMacroAuto and
//$LoadAddressMacroAuto_Padded 			implementations
//
//	Specialized instruction to populate the load address macro based on a
// project 	name.
//-----------------------------------------------------------------------------
void Internal_LoadAddressMacroAuto(bool bPad) {
  ...
  while (1) {
    ...
    if (!V_stricmp(pToken, "}")) {
      ...
      if (!g_pVPC->GetScript().ParsePropertyValue(NULL, szLength,
                                                  sizeof(szLength))) {
        continue;
      }
      if (strchr(szLength, '.')) {
      ...
      if (dllLength == 0) {
        g_pVPC->VPCSyntaxError(
            "$LoadAddressMacroAuto no longer supports 0 size dlls. Use "
            "$LoadAddressMacroAlias to have two orthogonal projects load in "
            "the same space");
      }
  ...
  if (bPad && pMacro) {
    ...
    if ((iInitialBaseAddress >= 0x82000000) &&
        (iInitialBaseAddress < 0x8C000000)) {
...
//
//	Specialized instruction to populate the load address macro based on a
// project 	name. Assumes the contained list is minimally packed and has
// free reign of 	space up to the limit. Finds unused space spreads it out
// evenly between 	each project
//-----------------------------------------------------------------------------
...
//
// Sets a conditional that can be used later in the script to affect later
// keywords. This works in both project and group scripts.
//-----------------------------------------------------------------------------
void VPC_SharedKeyword_Conditional() {
  ...
  conditional_t *pConditional =
      g_pVPC->FindOrCreateConditional(name, true, CONDITIONAL_CUSTOM);
  if (pConditional->type != CONDITIONAL_CUSTOM) {
    // scripts cannot change any platform or game conditionals
    g_pVPC->VPCSyntaxError("$Conditional cannot be used on the reserved '$%s'",
                           pConditional->upperCaseName.Get());
  }

  char environmentValue[MAX_SYSTOKENCHARS];
  if (Sys_EvaluateEnvironmentExpression(value, "0", environmentValue,
                                        sizeof(environmentValue))) {
    V_strncpy(value, environmentValue, sizeof(value));
  }
...
void VPC_PrepareToReadScript(const char *pInputScriptName, int depth,
                             bool bQuiet, char *&pScriptBuffer,
  ...
  g_pVPC->AddScriptToCRCCheck(
      szScriptName, CRC32_ProcessSingleBuffer(pScriptBuffer, scriptLen));
  ...
  if (bDoCRCCheck && g_pVPC->EvaluateConditionalExpression("$WINDOWS")) {
    ...
    for (intp i = 0; i < configurationNames.Count(); i++) {
      ...
      g_pVPC->GetProjectGenerator()->StartPropertySection(
          KEYWORD_CUSTOMBUILDSTEP, &bShouldSkip);
...
void VPC_HandleIncludeStatement(int depth, bool bQuiet,
                                void (*CallbackFn)(const char *pScriptName,
                                                   int depth, bool bQuiet)) {
  char szBigBuffer[MAX_SYSTOKENCHARS];
  if (g_pVPC->GetScript().ParsePropertyValue(NULL, szBigBuffer,
                                             sizeof(szBigBuffer))) {
    // recurse into and run
    char *pScriptBuffer;
    char szFixedScriptName[MAX_PATH];
    VPC_PrepareToReadScript(szBigBuffer, depth + 1, bQuiet, pScriptBuffer,
                            szFixedScriptName);
...
void WriteCRCCheckFile(const char *pVCProjFilename) {
  char szFilename[MAX_PATH];
  V_snprintf(szFilename, sizeof(szFilename), "%s." VPCCRCCHECK_FILE_EXTENSION,
             pVCProjFilename);
  ...
  char vpcExeRelPath[MAX_PATH];
  vpcExeRelPath[0] = '\0';
  if (V_MakeRelativePath(vpcExeAbsPath, g_pVPC->GetProjectPath(), vpcExeRelPath,
                         sizeof(vpcExeRelPath))) {
  ...
  for (intp i = 0; i < g_pVPC->m_ScriptList.Count(); i++) {
    ...
    if (filenameDict.Find(pScript->m_scriptName.String()) ==
        filenameDict.InvalidIndex()) {
      filenameDict.Insert(pScript->m_scriptName.String(), 1);

      // [crc] [filename]
      fprintf(fp, "%8.8x %s\n", (unsigned int)pScript->m_crc,
              pScript->m_scriptName.Get());
    }
  }
...
//
// This is called when it's done parsing a project. If there were any
// $SchemaFile entries in the project, then we will make this project depend on
// schemacompiler (via $AdditionalProjectDependencies).
//
// This fixes full rebuild problems where it's building a project that uses
// schemacompiler at the same time as it's building schemacompiler. This usually
...
void VPC_ForceAdditionalSchemaDependencies(const char *) {
  ...
  for (intp i = 0; i < configurationNames.Count(); i++) {
    ...
    g_pVPC->GetProjectGenerator()->HandleProperty(
        g_pOption_AdditionalProjectDependencies, "$BASE;SchemaCompiler");
...
void VPC_Keyword_Project(int depth, bool bQuiet) {
  ...
  // the unnamed project does not get written, once it is named it will be
  // written on closing scope
  if (V_stricmp(szProjectName, "UNNAMED")) {
    ...
    // need to do this at the end of parsing the project, so we have all the
    // macros needed
...
void VPC_Keyword_CustomBuildStep(void) {
  ...
  while (1) {
    ...
    if (pToken[0] == '[') {
      if (extensions.Count() == 0) {
        g_pVPC->VPCSyntaxError(
            "Conditional specified on a $CustomBuildStep without any "
            "extensions preceding it.");
      }
    ...
    if (VPC_IsBuiltInFileType(pToken)) {
      g_pVPC->VPCSyntaxError(
          "Cannot define custom build steps for built in file type: %s",
          pToken);
    }
  ...
  if (!pToken || !pToken[0] || V_strcmp(pToken, "{")) {
    ...
    if (g_pVPC->GetScript().GetData() > pScriptSave) {
      ...
      tempString.SetDirect(
          pScriptSave, int(g_pVPC->GetScript().GetData() - pScriptSave - 1));
...
void VPC_ParseProjectScriptParameters([[maybe_unused]] const char *szScriptName,
  ...
  while (1) {
    ...
    if (!V_stricmp(pToken, "$include")) {
      VPC_HandleIncludeStatement(depth, bQuiet,
                                 VPC_ParseProjectScriptParameters);
    } else if (!V_stricmp(pToken, "$configuration")) {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
bool CVPC::ParseProjectScript(const char *pScriptName, int depth, bool bQuiet,
                              bool bWriteCRCCheckFile) {
  char *pScriptBuffer;
  char szScriptName[MAX_PATH];

  VPC_PrepareToReadScript(pScriptName, depth, bQuiet, pScriptBuffer,
                          szScriptName);
  ...
  if (!depth) {
    ...
    // $(ProjectName) can be auto-decorated, making it unusable by scripts
    // expecting a pure project name
    FindOrCreateMacro("PROJECTNAME", true, g_pVPC->GetProjectName());

    // create reserved $LOADADDRESSNAME - defaults to project name but can be
    // aliased with $LoadAddressMacroAlias
    FindOrCreateMacro("LOADADDRESSNAME", true, g_pVPC->GetLoadAddressName());

    // create reserved $CRCCHECK
    // The CRCs themselves are dumped into theproject.vcproj.vpc_crc (in
    // VPC_WriteCRCCheckFile), so all this does is point vpccrccheck.exe at it
    // with "vpccrccheck.exe -crc2 theproject.vcproj" scripts add the terminal
    // /n if they append, after referencing $CRCCHECK needs to be quoted to work
    // with /dp which puts " (platform) " in project names,

    CUtlString crcCheckString;
    crcCheckString =
        "if exist $QUOTE$SRCDIR\\devtools\\bin\\" VPCCRCCHECK_EXE_FILENAME
        "$QUOTE $QUOTE$SRCDIR\\devtools\\bin\\" VPCCRCCHECK_EXE_FILENAME
        "$QUOTE -crc2 $QUOTE";
  ...
  if (!depth) {
    ...
    if (bWriteCRCCheckFile &&
        g_pVPC->GetMissingFilesCount() == cMissingFilesPreParse) {
      // Finally write out the file with all the CRCs in it. This is referenced
      // by the $CRCCHECK macro in the prebuild steps.
      WriteCRCCheckFile(g_pVPC->GetOutputFilename());
    }

    g_pVPC->m_ScriptList.Purge();
    g_pVPC->RemoveScriptCreatedMacros();  // Remove any macros that came from
                                          // the script file.
  }
...
//
// Temporary - Once the schema workflow settles down, this schema-specific code
// should be
//             at minimum moved into its own file, and ideally generalized so
//             that VPC has a minimum of embedded schema-specific logic.
//
//
//
//////////////////////////////////////////////////////////////////////////////////////////
...
void VPC_FakeKeyword_SchemaFolder(CBaseProjectDataCollector *pDataCollector) {
  ...
  if (pMacro && atoi(pMacro->value.Get())) return;
  ...
  g_pVPC->ResolveMacrosInString("$SRCDIR/devtools/bin/schemacompiler.dll",
                                schemaCompilerPath, sizeof(schemaCompilerPath));
  ...
  for (intp i = 0; i < g_pVPC->m_SchemaFiles.Count(); i++) {
    ...
    V_strncpy(fileInfo.szFile, g_pVPC->m_SchemaFiles[i],
              sizeof(fileInfo.szFile));
    ...
    if (IsCFileExtension(szExt)) {
      ...
      V_snprintf(szGenRelPath, sizeof(szGenRelPath),
                 "generated_code_%s_%s/%s_schema_cpp_gen.cpp",
                 g_pVPC->GetProjectName(), pPlatformName, szBase);
    } else {
      fileInfo.bIsCppFile = false;
      V_snprintf(szGenRelPath, sizeof(szGenRelPath),
                 "generated_code_%s_%s/%s_schema_gen.cpp",
                 g_pVPC->GetProjectName(), pPlatformName, szBase);
    }

    V_strncpy(fileInfo.szGeneratedFile, szGenRelPath,
              sizeof(fileInfo.szGeneratedFile));
  ...
  V_snprintf(szSchemaOutAnchorPath, sizeof(szSchemaOutAnchorPath),
             "generated_code_%s_%s/%s_schema_anchor.cpp",
             g_pVPC->GetProjectName(), pPlatformName, g_pVPC->GetProjectName());
  V_FixSlashes(szSchemaOutAnchorPath, '/');
  schemaOutputs += szSchemaOutAnchorPath;
  schemaOutputs += ";";

  char szSchemaPath[MAX_PATH];
  V_snprintf(szSchemaPath, sizeof(szSchemaPath), "%s_%s.schproj",
             g_pVPC->GetProjectName(), pPlatformName);
  ...
  g_pVPC->ResolveMacrosInString("$SRCDIR/devtools/bin/schemacompiler.exe",
                                schemaCompilerExePath,
                                sizeof(schemaCompilerExePath));
  ...
  vpcBuffer.Printf(
      ...
      "		$CustomBuildStep\n"
      "		{\n"
      "			$CommandLine	\"%s -schproj $(InputPath) -config "
      "$(ConfigurationName)\"\n"
      "			$Description	\"Schema Compiler for "
      "[$(ConfigurationName)] $(InputName)\"\n"
      "			$Outputs		\"%s\"\n"
      "			$AdditionalDependencies		\"%s\"\n"
      "		}\n"
      "	}\n"
      "}\n",
      szSchemaPath, schemaCompilerExePath, schemaOutputs.Get(),
      schemaInputs.Get());
  ...
  static const char *schemaConfiguration =
      ...
      "			$Create/UsePrecompiledHeader		\"Not Using "
      "Precompiled Headers\"\n"
      "		}\n"
      "	}\n"
      "}\n";
  ...
  g_pVPC->SetIgnoreRedundancyWarning(true);
  g_pVPC->GetScript().PushScript("Internal List [Schema]",
                                 (char *)vpcBuffer.Base());
  ...
  g_pVPC->ResolveMacrosInString("$SRCDIR\\public", dmeTargetFolder,
                                sizeof(dmeTargetFolder));
  ...
  for (int i = pDataCollector->m_BaseConfigData.m_Configurations.First();
    ...
    CSpecificConfig *pConfig =
        pDataCollector->m_BaseConfigData.m_Configurations[i];
...
KeyValues *ConfigPreprocessorSettingsAsKV(CSpecificConfig *pConfig) {
...

  {
    ...
    CSplitString outStrings(
        pInConfigKV->GetString(g_pOption_PreprocessorDefinitions),
    ...
    for (intp i = 0; i < g_pVPC->m_Macros.Count(); i++) {
      ...
      if (pMacro->m_bSetupDefineInProjectFile) {
        V_snprintf(szNum, sizeof(szNum), "%03d", nDefine++);
        V_snprintf(pBuf, sizeof(pBuf), "%s=%s", pMacro->name.Get(),
                   pMacro->value.Get());
