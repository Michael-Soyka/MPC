...
bool CProjectFile::GetConfiguration(const char *pConfigName,
                                    CProjectConfiguration **ppConfig) {
...
bool CProjectFile::AddConfiguration(const char *pConfigName,
                                    CProjectConfiguration **ppConfig) {
  if (ppConfig) {
    // assume not found
    *ppConfig = NULL;
...
bool CProjectFolder::GetFolder(const char *pFolderName,
                               CProjectFolder **pFolder) {
  if (pFolder) {
    // assume not found
    *pFolder = NULL;
  }
...
bool CProjectFolder::AddFolder(const char *pFolderName,
                               CProjectFolder **pFolder) {
  if (pFolder) {
    // assume not added
    *pFolder = NULL;
...
void CProjectFolder::AddFile(const char *pFilename, CProjectFile **ppFile) {
  ...
  for (iIndex = m_Files.Head(); iIndex != m_Files.InvalidIndex();
    ...
    if (V_stricmp(V_GetFileName(pFilename),
                  V_GetFileName(m_Files[iIndex]->m_Name.Get())) < 0) {
...
bool CPropertyStateLessFunc::Less(const intp &lhs, const intp &rhs,
                                  void *pContext) {
  int lhsPropertyId = ((CPropertyStates *)pContext)
                          ->m_Properties[lhs]
                          .m_pToolProperty->m_nPropertyId;
  int rhsPropertyId = ((CPropertyStates *)pContext)
                          ->m_Properties[rhs]
                          .m_pToolProperty->m_nPropertyId;
...
PropertyState_t *CPropertyStates::GetProperty(const char *pPropertyName) {
  ...
  for (intp i = 0; i < m_Properties.Count(); i++) {
    ...
    const char *pLegacyString =
        m_Properties[i].m_pToolProperty->m_LegacyString.Get();
    if (pLegacyString[0]) {
      if (pLegacyString[0] == '$') {
        pLegacyString++;
      }
...
bool CPropertyStates::SetStringProperty(ToolProperty_t *pToolProperty,
  ...
  if (!pCurrentValue && pRootTool) {
    ...
    for (intp i = 0; i < pRootTool->m_PropertyStates.m_Properties.Count(); i++) {
      if (pRootTool->m_PropertyStates.m_Properties[i].m_pToolProperty ==
          pToolProperty) {
        pCurrentValue =
            pRootTool->m_PropertyStates.m_Properties[i].m_StringValue.Get();
        break;
      }
    }
  }
  ...
  if (!g_pVPC->GetScript().ParsePropertyValue(pCurrentValue, buff,
                                              sizeof(buff)))
    return true;
  ...
  if (pCurrentValue && !V_stricmp(pCurrentValue, buff)) {
    g_pVPC->VPCWarning("%s matches default setting, [%s line:%d]",
                       pToolProperty->m_ParseString.Get(),
                       g_pVPC->GetScript().GetName(),
                       g_pVPC->GetScript().GetLine());
  }
...
bool CPropertyStates::SetListProperty(ToolProperty_t *pToolProperty,
                                      CProjectTool *pRootTool) {
  char buff[MAX_SYSTOKENCHARS];
  ...
  if (!pNewOrdinalValue) {
    g_pVPC->VPCSyntaxError("Unknown Ordinal for %s",
                           pToolProperty->m_ParseString.Get());
  }
  ...
  if (pCurrentOrdinalValue &&
      !V_stricmp(pCurrentOrdinalValue, pNewOrdinalValue)) {
    g_pVPC->VPCWarning("%s matches default setting, [%s line:%d]",
                       pToolProperty->m_ParseString.Get(),
                       g_pVPC->GetScript().GetName(),
                       g_pVPC->GetScript().GetLine());
...
bool CPropertyStates::SetBoolProperty(ToolProperty_t *pToolProperty,
                                      CProjectTool *pRootTool, bool bEnabled) {
...
bool CPropertyStates::SetIntegerProperty(ToolProperty_t *pToolProperty,
                                         CProjectTool *pRootTool) {
  char buff[MAX_SYSTOKENCHARS];
  if (!g_pVPC->GetScript().ParsePropertyValue(NULL, buff, sizeof(buff)))
    return true;
...
bool CPropertyStates::SetProperty(ToolProperty_t *pToolProperty,
                                  CProjectTool *pRootTool) {
  bool bHandled = false;
  switch (pToolProperty->m_nType) {
    ...
    case PT_DEPRECATED:
      g_pVPC->VPCError(
          "SetProperty: Property %s has been deprecated and is no longer "
          "supported!",
          pToolProperty->m_ParseString.Get());
      break;
    ...
    default:
      g_pVPC->VPCError(
          "SetProperty: Unknown type for %s - requires implementation",
          pToolProperty->m_ParseString.Get());
  }
...
static bool FilesSortLessFunc(CProjectFile *const &pLHS,
                              CProjectFile *const &pRHS) {
...
bool CProjectConfiguration::IsEmpty() {
  if (m_PropertyStates.m_Properties.Count()) return false;

  if (m_pDebuggingTool &&
      m_pDebuggingTool->m_PropertyStates.m_Properties.Count())
    return false;
  ...
  if (m_pLibrarianTool &&
      m_pLibrarianTool->m_PropertyStates.m_Properties.Count())
    return false;
  ...
  if (m_pXMLDocGenTool &&
      m_pXMLDocGenTool->m_PropertyStates.m_Properties.Count())
    return false;

  if (m_pBrowseInfoTool &&
      m_pBrowseInfoTool->m_PropertyStates.m_Properties.Count())
    return false;

  if (m_pResourcesTool &&
      m_pResourcesTool->m_PropertyStates.m_Properties.Count())
    return false;

  if (m_pPreBuildEventTool &&
      m_pPreBuildEventTool->m_PropertyStates.m_Properties.Count())
    return false;

  if (m_pPreLinkEventTool &&
      m_pPreLinkEventTool->m_PropertyStates.m_Properties.Count())
    return false;

  if (m_pPostBuildEventTool &&
      m_pPostBuildEventTool->m_PropertyStates.m_Properties.Count())
    return false;
...
bool CProjectConfiguration::SetProperty(ToolProperty_t *pToolProperty) {
  ...
  // have to mimic what the COM layer used to do which is to configure itself
  // based on the type of application its building VPC enforces a strict order,
  // configuration blocks must come before any tool block to allow this to be
  // rational
  if (bHandled &&
      !V_stricmp(pToolProperty->m_ParseString, "$ConfigurationType")) {
    PropertyState_t *pPropertyState =
        m_PropertyStates.GetProperty(pToolProperty->m_nPropertyId);
    if (pPropertyState &&
        ((V_stristr(pPropertyState->m_OrdinalString.Get(), "static library") ||
          !V_stricmp(pPropertyState->m_OrdinalString.Get(), "LIB")))) {
...
bool CProjectTool::SetProperty(ToolProperty_t *pToolProperty,
                               CProjectTool *pRootTool) {
...
CRelevantPropertyNames g_RelevantSchemaPropertyNames = {
    g_pRelevantSchemaProperties, V_ARRAYSIZE(g_pRelevantSchemaProperties)};
...
void CVCProjGenerator::Clear() {
  ...
  CProjectConfiguration *pDebugConfig =
      new CProjectConfiguration(this, "Debug", NULL);
  m_RootConfigurations.AddToTail(pDebugConfig);

  CProjectConfiguration *pReleaseConfig =
      new CProjectConfiguration(this, "Release", NULL);
  m_RootConfigurations.AddToTail(pReleaseConfig);
}

void CVCProjGenerator::SetupGeneratorDefinition(
    IVCProjWriter *pVCProjWriter, const char *pDefinitionName,
...
#error( "NEEDS TO BE FIXED" )
  ...
  g_pVPC->VPCStatus(true, "Saving... Project: '%s' File: '%s'",
                    GetProjectName().String(), g_pVPC->GetOutputFilename());
  ...
  if (!bValid) {
    g_pVPC->VPCError("Cannot save the specified project '%s' to '%s'",
                     GetProjectName().Get(), m_OutputFilename.Get());
  }
...
void CVCProjGenerator::SetGUID(const char *pOutputFilename) {
  ...
  // set the GUID
  m_GUIDString = Sys_GuidFromFileName(szBasename);
}

CUtlString CVCProjGenerator::GetProjectName() { return m_ProjectName; }
...
bool CVCProjGenerator::StartFile(const char *pFilename,
  ...
  // remove any incoming surrounding quotes, this only causes string handling
  ...
  if (m_spFolderStack.Count() == 0) {
    ...
    // add at subfolder
    AddFileToFolder(pFilename, m_spFolderStack.Top(), bWarnIfAlreadyExists,
                    &pFile);
  }
...
void CVCProjGenerator::EndFile() { BaseClass::EndFile(); }
...
void CVCProjGenerator::StartConfigurationBlock(const char *pConfigName,
                                               bool bFileSpecific) {
  ...
  if (bFileSpecific) {
    ...
    if (!bValid) {
      ...
      if (!IsConfigurationNameValid(pConfigName)) {
        g_pVPC->VPCError("File %s, Unknown configuration '%s'",
                         m_pProjectFile->m_Name.Get(), pConfigName);
      }

      bValid = m_pProjectFile->AddConfiguration(pConfigName, &pFileConfig);
      if (!bValid) {
        g_pVPC->VPCError("File %s, Could not get file configuration '%s'",
                         m_pProjectFile->m_Name.Get(), pConfigName);
      }
    }
...
void CVCProjGenerator::EndConfigurationBlock() {
  ...
  if (m_pFileConfig && m_pFileConfig->IsEmpty()) {
    // any file configuration (after parsing) that has no property state gets
    // purged
...
void CVCProjGenerator::FileExcludedFromBuild(bool bExcluded) {
  if (!m_pFileConfig) {
    g_pVPC->VPCSyntaxError(
        "Cannot set $ExcludedFromBuild unless in a $File configuration "
        "context");
  }

  BaseClass::FileExcludedFromBuild(bExcluded);

  ToolProperty_t *pToolProperty = m_pGeneratorDefinition->GetProperty(
      KEYWORD_GENERAL, "$ExcludedFromBuild");
  if (!pToolProperty) {
...
bool CVCProjGenerator::StartPropertySection(configKeyword_e eKeyword,
                                            bool *pbShouldSkip) {
  ...
  switch (eKeyword) {
    ...
    case KEYWORD_MANIFEST:
      if (!(g_pVPC->IsPlatformDefined("WIN32") ||
            g_pVPC->IsPlatformDefined("WIN64"))) {
        // windows specific
        break;
      }
...
void CVCProjGenerator::HandleProperty(const char *pPropertyName,
  ...
  if (pCustomScriptData) {
    g_pVPC->GetScript().PushScript("HandleProperty custom data",
                                   pCustomScriptData);
  }

  ToolProperty_t *pToolProperty = m_pGeneratorDefinition->GetProperty(
      m_nActivePropertySection, pPropertyName);
  if (!pToolProperty) {
    // unknown property
  ...
  switch (m_nActivePropertySection) {
    ...
    default:
      g_pVPC->VPCError(
          "HandleProperty: No support for Tool:%s Property:%s - requires "
          "implementation",
          g_pVPC->KeywordToName(m_nActivePropertySection), pPropertyName);
  }
...
bool CVCProjGenerator::GetFolder(const char *pFolderName,
                                 CProjectFolder *pParentFolder,
                                 CProjectFolder **ppOutFolder) {
  bool bValid;
  if (!pParentFolder) {
...
bool CVCProjGenerator::AddFolder(const char *pFolderName,
                                 CProjectFolder *pParentFolder,
                                 CProjectFolder **ppOutFolder) {
  bool bValid;
  if (!pParentFolder) {
...
void CVCProjGenerator::AddFileToFolder(const char *pFilename,
                                       CProjectFolder *pFolder,
                                       bool bWarnIfAlreadyExists,
                                       CProjectFile **ppFile) {
...
bool CVCProjGenerator::RemoveFileFromFolder(const char *pFilename,
                                            CProjectFolder *pFolder) {
  bool bFound = false;
  CProjectFile findProjectFile(this, pFilename);
...
bool CVCProjGenerator::GetRootConfiguration(const char *pConfigName,
                                            CProjectConfiguration **ppConfig) {
  if (!pConfigName || !pConfigName[0]) {
    g_pVPC->VPCError("Empty or bad configuration name.");
  }
...
void CVCProjGenerator::ApplyInternalPreprocessorDefinitions() {
  ...
  for (intp i = 0; i < m_RootConfigurations.Count(); i++) {
    ...
    if (pCompilerTool) {
      PropertyState_t *pPropertyState =
          pCompilerTool->m_PropertyStates.GetProperty(
  ...
  for (int iIndex = m_FileDictionary.FirstInorder();
