...
bool IsSharedLibraryFile(const char *pFilename) {
  ...
  if (pExt && (V_stricmp(pExt, "so") == 0 || V_stricmp(pExt, "dylib") == 0 ||
...
bool IsLibraryFile(const char *pFilename) {
  const char *pExt = V_GetFileExtension(pFilename);
  if (IsSharedLibraryFile(pFilename) ||
      (pExt && (V_stricmp(pExt, "lib") == 0 || V_stricmp(pExt, "a") == 0))) {
    return true;
  } else {
    return false;
  }
...
bool CDependency::FindDependency_Internal(
    CUtlVector<CUtlBuffer> &callTreeOutputStack, CDependency *pTest, int flags,
    int depth) {
  if (pTest == this) return true;
  ...
  if (!(flags & k_EDependsOnFlagTraversePastLibs) &&
      m_Type == k_eDependencyType_Library)
    return false;
...
CDependency_Project::CDependency_Project(
    CProjectDependencyGraph *pDependencyGraph)
...
void CDependency_Project::StoreProjectParameters(const char *szScriptName) {
  ...
  V_GetCurrentDirectory(m_szStoredCurrentDirectory,
                        sizeof(m_szStoredCurrentDirectory));
...
void CDependency_Project::ExportProjectParameters() {
  ...
  if (m_StoredConditionalsActive.Count() > g_pVPC->m_Conditionals.Count()) {
    g_pVPC->VPCError("ExportProjectParameters( %s ) - too many defines stored.",
...
class CSingleProjectScanner : public CBaseProjectDataCollector {
 public:
  ...
  CSingleProjectScanner()
      : CBaseProjectDataCollector(&g_DependencyRelevantPropertyNames) {
    m_bInLinker = false;
  }

  virtual void EndProject() {}

  void ScanProjectFile(CProjectDependencyGraph *pGraph,
                       const char *szScriptName,
                       CDependency_Project *pProject) {
    // Someday we'll pass this interface down into VPC_ParseProjectScript
    // instead of using the global.
    IBaseProjectGenerator *pOldGenerator = g_pVPC->GetProjectGenerator();
    g_pVPC->SetProjectGenerator(this);

    // This has VPC parse the script and CBaseProjectDataCollector collects all
    // the data into lists of the stuff we care about like source files and
    // include paths.
  ...
  void SetupFilesList(CProjectDependencyGraph *pGraph,
                      CDependency_Project *pProject) {
    for (int i = m_Files.First(); i != m_Files.InvalidIndex();
      ...
      if (pFile->m_Configurations.Count() > 0) {
        ...
        for (int iSpecific = pFile->m_Configurations.First();
          ...
          if (pTest->m_bFileExcluded && !pTest->m_bIsSchema) ++nExcluded;
        }
      ...
      // Don't bother with source files if we're not building the full
      // dependency set.
      if (!pGraph->m_bFullDependencySet)
        if (IsSourceFile(sAbsolutePath)) continue;

      // For source files, don't bother with files that don't exist. If we do
      // create entries for files that don't exist, then they'll have a "cache
      // file size"
  ...
  void AddIncludesForFile(CProjectDependencyGraph *pGraph, CDependency *pFile) {
    ...
    if (pFile->m_bCheckedIncludes) return;
    ...
    if (!V_ExtractFilePath(pFile->GetName(), szDir, sizeof(szDir)))
      g_pVPC->VPCError("AddIncludesForFile: V_ExtractFilePath( %s ) failed.",
                       pFile->GetName());

    includeDirs.AddToTail(szDir);
    includeDirs.AddMultipleToTail(m_IncludeDirectories.Count(),
                                  m_IncludeDirectories.Base());
    ...
    for (intp iIncludeFile = 0; iIncludeFile < includes.Count();
      ...
      for (intp iIncludeDir = 0; iIncludeDir < includeDirs.Count();
        ...
        V_ComposeFileName(includeDirs[iIncludeDir].String(),
                          includes[iIncludeFile].String(), szFullName,
                          sizeof(szFullName));

        CDependency *pIncludeFile = pGraph->FindDependency(szFullName);
        if (!pIncludeFile) {
          if (!Sys_Exists(szFullName)) continue;
  ...
  void GetIncludeFiles(const char *pFilename,
                       CUtlVector<CUtlString> &includes) {
    char *pFileData;
    ...
    if (ret == -1) {
      if (g_pVPC->IsVerbose()) {
        g_pVPC->VPCWarning(
            "GetIncludeFiles( %s ) - can't open file (included by project %s).",
            pFilename, m_ScriptName.String());
      }
      return;
    }
    ...
    while (1) {
      ...
      if (!pIncludeStatement) break;

      pSearchPos = pIncludeStatement + V_strlen(pLookFor);

      if (!SeekToIncludeStart(pSearchPos)) continue;
      const char *pFilenameStart = pSearchPos;

      if (!SeekToIncludeEnd(pSearchPos)) continue;
      ...
      V_strncpy(szIncludeFilename, pFilenameStart,
                pFilenameEnd - pFilenameStart + 1);
  ...
  void SetupIncludeDirectories(CSpecificConfig *pConfig,
                               const char *szScriptName) {
    if (m_BaseConfigData.m_Configurations.Count() == 0)
      g_pVPC->VPCError("No configurations for %s in project %s.", szScriptName,
                       m_ScriptName.String());

    const char *pIncludes =
        pConfig->m_pKV->GetString(g_pOption_AdditionalIncludeDirectories, "");
    CSplitString relativeIncludeDirs(pIncludes,
  ...
  void SetupImportLibrary([[maybe_unused]] CProjectDependencyGraph *pGraph,
                          CSpecificConfig *pConfig,
                          [[maybe_unused]] const char *szScriptName) {
    ...
    if (!m_LinkerOutputFile.Length()) {
      m_LinkerOutputFile =
          pConfig->m_pKV->GetString(g_pOption_OutputFile, NULL);
    }
  }

  void SetupAdditionalProjectDependencies(CDependency_Project *pProject,
                                          CSpecificConfig *pConfig) {
    const char *pVal =
        pConfig->m_pKV->GetString(g_pOption_AdditionalProjectDependencies);
    if (pVal) {
  ...
  void SetupAdditionalOutputFiles(CDependency_Project *pProject,
                                  CSpecificConfig *pConfig) {
    const char *pVal =
        pConfig->m_pKV->GetString(g_pOption_AdditionalOutputFiles);
    if (pVal) {
 ...
 protected:
  ...
  virtual void HandleProperty(const char *pProperty,
                              const char *pCustomScriptData) {
    // We don't want the $OutputFile property from the $BrowseInformation
    // section.
...
void CProjectDependencyGraph::BuildProjectDependencies(
    int nBuildProjectDepsFlags,
    CUtlVector<CDependency_Project *> *pPhase1Projects) {
  m_bFullDependencySet =
      ((nBuildProjectDepsFlags & BUILDPROJDEPS_FULL_DEPENDENCY_SET) != 0);
  m_nFilesParsedForIncludes = 0;
  ...
  if (nBuildProjectDepsFlags & BUILDPROJDEPS_CHECK_ALL_PROJECTS) {
    ...
    projectList.AddMultipleToTail(g_pVPC->m_TargetProjects.Count(),
                                  g_pVPC->m_TargetProjects.Base());
  }
  ...
  // Save the expensive work we did into a cache file so it can be used next
  // time.
  if (m_bFullDependencySet) {
  ...
  for (intp iMainProject = 0; iMainProject < m_Projects.Count();
    ...
    for (intp i = 0; i < pMainProject->m_AdditionalProjectDependencies.Count();
      ...
      if (j == m_Projects.Count()) {
        // VPCError( "Project %s lists '%s' in its
        // $AdditionalProjectDependencies, but there is no project by that
        // name.", pMainProject->GetName(), pLookingFor );
        continue;
      }

      if (pMainProject->m_AdditionalDependencies.Find(m_Projects[j]) ==
          pMainProject->m_AdditionalDependencies.InvalidIndex())
        pMainProject->m_AdditionalDependencies.AddToTail(m_Projects[j]);
    }

    if (pPhase1Projects != NULL) {
      //
      // See if there's a project in phase 1 built from the same VPC, and, if
      // so, add it as a dependency. This prevents a bunch of race conditions
      // when projects are built in a distributed fashion (ala Incredibuild) and
      // the projects step on each other.
      //
      ...
      if (pMainProject->m_AdditionalDependencies.Find((*pPhase1Projects)[j]) ==
          pMainProject->m_AdditionalDependencies.InvalidIndex()) {
...
bool CProjectDependencyGraph::VisitProject(projectIndex_t iProject,
  ...
  // Remember various parameters passed to us so we can regenerate this project
  // without having to call VPC_IterateTargetProjects.
  pProject->StoreProjectParameters(szProjectName);

  char sAbsProjectFilename[MAX_PATH];
  V_MakeAbsolutePath(sAbsProjectFilename, sizeof(sAbsProjectFilename),
  ...
  // Get a list of all files that depend on this project, starting with the .lib
  // if it generates one.
  ...
  // $(ImportLibrary) will be a lib in the case of DLLs that create libs (like
  // tier0).
  ...
  for (intp i = 0; i < outputFiles.Count(); i++) {
    ...
    V_StrSubst(pFilename, "$(TargetName)", sTargetNameReplacement, sReplaced,
               sizeof(sReplaced));
...
void CProjectDependencyGraph::GetProjectDependencyTree(
    projectIndex_t iProject, CUtlVector<projectIndex_t> &dependentProjects,
    bool bDownwards) {
  ...
  for (intp i = 0; i < m_Projects.Count(); i++) {
    ...
    for (intp iOther = 0; iOther < m_Projects.Count(); iOther++) {
      ...
      if (bDownwards)
        bThereIsADependency = pProject->DependsOn(
            pOther, k_EDependsOnFlagCheckNormalDependencies |
                        k_EDependsOnFlagCheckAdditionalDependencies |
                        k_EDependsOnFlagRecurse |
                        k_EDependsOnFlagTraversePastLibs);
      else
        bThereIsADependency = pOther->DependsOn(
            pProject, k_EDependsOnFlagCheckNormalDependencies |
                          k_EDependsOnFlagCheckAdditionalDependencies |
                          k_EDependsOnFlagRecurse |
                          k_EDependsOnFlagTraversePastLibs);

      if (bThereIsADependency) {
        if (dependentProjects.Find(pOther->m_iProjectIndex) ==
            dependentProjects.InvalidIndex())
...
CDependency *CProjectDependencyGraph::FindOrCreateDependency(
  ...
  if (pDependency) return pDependency;
  ...
  Sys_FileInfo(pFilename, pDependency->m_nCacheFileSize,
               pDependency->m_nCacheModificationTime);
...
void CProjectDependencyGraph::ClearAllDependencyMarks() {
  if (m_iDependencyMark == 0xFFFFFFFF) {
    m_iDependencyMark = 1;
    for (int i = m_AllFiles.First(); i != m_AllFiles.InvalidIndex();
         i = m_AllFiles.Next(i)) {
...
bool CProjectDependencyGraph::LoadCache(const char *pFilename) {
  ...
  while (1) {
    byte bMore;
    if (fread(&bMore, 1, 1, fp) != 1 || bMore == 0) break;
...
bool CProjectDependencyGraph::SaveCache(const char *pFilename) {
  ...
  for (int i = m_AllFiles.First(); i != m_AllFiles.InvalidIndex();
    ...
    fwrite(&pDep->m_nCacheModificationTime,
    ...
    for (intp iDependency = 0; iDependency < pDep->m_Dependencies.Count();
         iDependency++) {
...
void CProjectDependencyGraph::RemoveDirtyCacheEntries() {
  // NOTE: This could be waaaay more efficient by pointing files at their
  // parents and removing all the way up the chain rather than iterating over
  // and over but this keeps the data structures simple.
  bool bAnyDirty = true;
  while (bAnyDirty) {
    bAnyDirty = false;
    ...
    for (int i = m_AllFiles.First(); i != m_AllFiles.InvalidIndex();
         i = m_AllFiles.Next(i)) {
      CDependency *pDep = m_AllFiles[i];
      if (pDep->m_bCacheDirty) continue;
  ...
  // Now that any dirty children have flagged their parents as dirty, we can
  // remove them.
  int iNext;
...
class CGameFilterProjectIterator : public IProjectIterator {
 public:
  ...
  virtual bool VisitProject(projectIndex_t, const char *szProjectName) {
    ...
    if (!bAdded) {
      g_pVPC->VPCError(
          "CGameFilterProjectIterator::VisitProject( %s ) - no project found "
          "by that name.",
          szProjectName);
    }
...
void CProjectDependencyGraph::TranslateProjectIndicesToDependencyProjects(
    CUtlVector<projectIndex_t> &projectList,
    CUtlVector<CDependency_Project *> &out) {
