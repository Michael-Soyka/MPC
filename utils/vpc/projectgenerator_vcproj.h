...
class CProjectFile {
 public:
  ...
  bool GetConfiguration(const char *pConfigName,
                        CProjectConfiguration **ppConfig);
  bool AddConfiguration(const char *pConfigName,
                        CProjectConfiguration **ppConfig);
...
class CPropertyStateLessFunc {
 public:
  bool Less(const intp &lhs, const intp &rhs, void *pContext);
};
...
class CPropertyStates {
 ...
 private:
  bool SetStringProperty(ToolProperty_t *pToolProperty,
                         CProjectTool *pRootTool = NULL);
  bool SetListProperty(ToolProperty_t *pToolProperty,
                       CProjectTool *pRootTool = NULL);
  bool SetBoolProperty(ToolProperty_t *pToolProperty,
                       CProjectTool *pRootTool = NULL);
  bool SetBoolProperty(ToolProperty_t *pToolProperty, CProjectTool *pRootTool,
                       bool bEnabled);
  bool SetIntegerProperty(ToolProperty_t *pToolProperty,
                          CProjectTool *pRootTool = NULL);
};

class CProjectTool {
 public:
  CProjectTool(CVCProjGenerator *pGenerator) { m_pGenerator = pGenerator; }

  CVCProjGenerator *GetGenerator() { return m_pGenerator; }

  // when the property belongs to the root tool (i.e. linker), no root tool is
  // passed in when the property is for the file's specific configuration tool,
  // (i.e. compiler/debug), the root tool must be supplied
  virtual bool SetProperty(ToolProperty_t *pToolProperty,
                           CProjectTool *pRootTool = NULL);
...
class CVCProjGenerator : public CBaseProjectDataCollector {
 public:
  ...
  virtual void GetAllConfigurationNames(
      CUtlVector<CUtlString> &configurationNames);
  virtual void StartConfigurationBlock(const char *pConfigName,
                                       bool bFileSpecific);
  ...
  CGeneratorDefinition *GetGeneratorDefinition() {
    return m_pGeneratorDefinition;
  }
  void SetupGeneratorDefinition(IVCProjWriter *pVCProjWriter,
 ...
 private:
  ...
  bool GetFolder(const char *pFolderName, CProjectFolder *pParentFolder,
                 CProjectFolder **pOutFolder);
  // returns true if added, false otherwise (duplicate)
  bool AddFolder(const char *pFolderName, CProjectFolder *pParentFolder,
                 CProjectFolder **pOutFolder);

  // returns true if found, false otherwise
  bool FindFile(const char *pFilename, CProjectFile **pFile);
  void AddFileToFolder(const char *pFilename, CProjectFolder *pFolder,
                       bool bWarnIfExists, CProjectFile **pFile);
